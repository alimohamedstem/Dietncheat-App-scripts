function importDistributionData2() {
  ...
  var sourceSh = sourceSS.getSheetByName("DietUpdates"); // ورقة المصدر

  var targetSS = SpreadsheetApp.openById(targetId);
  var targetSh = targetSS.getSheetByName("DietUpdates2"); // ورقة الهدف

  // تحديد آخر صف
  var lastRow = sourceSh.getLastRow();

  // جلب البيانات من العمود C (3) إلى K (9 أعمدة)
  var data = sourceSh.getRange(1, 6, lastRow, 9).getValues(); // العمود 3 = C, عدد الأعمدة = 9
