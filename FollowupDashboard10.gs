function importDistributionData8() {
  ...
  var sourceSh = sourceSS.getSheetByName("Diet Update"); // ورقة المصدر

  var targetSS = SpreadsheetApp.openById(targetId);
  var targetSh = targetSS.getSheetByName("DietUpdate2"); // ورقة الهدف

  // تحديد آخر صف
  var lastRow = sourceSh.getLastRow();

  // جلب البيانات من العمود C (3) إلى K (9 أعمدة)
  var data = sourceSh.getRange(1, 1, lastRow, 11).getValues(); // العمود 3 = C, عدد الأعمدة = 9
